var questions = [
  {
    q: 'کدام دریچه بین دهلیز و بطن چپ قرار دارد؟',
    options: ['دریچه میترال', 'دریچه سه‌لختی', 'دریچه آئورت', 'دریچه پولمونر'],
    answer: 0
  },
  {
    q: 'ضربان‌ساز طبیعی قلب کجاست؟',
    options: ['گره AV', 'گره SA', 'دسته هیس', 'الیاف پورکنژ'],
    answer: 1
  },
  {
    q: 'کدام هورمون توسط غده تیروئید ترشح می‌شود؟',
    options: ['انسولین', 'کورتیزول', 'تیروکسین (T4)', 'آدرنالین'],
    answer: 2
  }
];
...
function renderQ() {
  ...
  q.options.forEach(function (opt, i) {
    ...
    html += '<label class="q-option' + sel + '" onclick="selectOpt(' + i + ')">' +
      '<input type="radio" name="q" ' + (answers[current] === i ? 'checked' : '') + ' /> ' + opt +
      '</label>';
  });
...
function submitExam() {
  ...
  document.getElementById('questionArea').innerHTML =
    ...
    '<h3>نتیجه آزمون</h3>' +
    '<p style="font-size:2.5rem;font-weight:700;color:#059669;margin:16px 0">' + percent + '٪</p>' +
    '<p>' + score + ' از ' + questions.length + ' پاسخ صحیح</p></div>';
}
